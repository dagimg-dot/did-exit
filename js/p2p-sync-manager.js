...
export class P2PSyncManager {
	...
	emit(event, data) {
		(this.events[event] || []).forEach((cb) => {
			try {
				...
				console.error(
					`Error in P2PSyncManager listener for ${event}:`,
					err,
				);
			}
		});
	...
	async createShareSession(pdfId) {
			...
			peer.on("signal", async (offer) => {
				if (!peer.__offerPublished) {
					try {
						...
						console.log(
							`${this.logPrefix} Offer successfully published.`,
						);
						resolve();
					} catch (err) {
						console.error(
							`${this.logPrefix} Failed to publish offer:`,
							err,
						);
						reject(err);
					}
		...
		this.#pollAnswer(roomUrl)
			.then((answer) => {
				console.log(
					`${this.logPrefix} Received answer. Connecting to peer...`,
				);
				peer.signal(answer);
			})
			.catch((err) => {
		...
		peer.on("close", () => {
			console.log(
				`${this.logPrefix} Peer connection closed for room:`,
				roomId,
			);
	...
	async joinShareSession(roomId) {
		console.log(
			`${this.logPrefix} Joining share session for room:`,
			roomId,
		);
		const roomUrl = `${window.location.origin}/.netlify/functions/webrtc-room/${roomId}`;
